...
function Checkout() {
  ...
  const enderecoSelecionadoCliente = useWatch({
    control: metodos.control,
    name: "endereco",
  });

  useEffect(() => {
    console.log(enderecoSelecionadoCliente);
  }, [enderecoSelecionadoCliente]);
  ...
  useEffect(() => {
    ...
    if (carrinho && carrinho.length > 0) {
      const valorProdutosNum = carrinho.reduce(
        (acc, item) =>
          acc +
          parseFloat(item.preco) * (parseInt(item.quantidadeCarrinho) || 1),
        0
      );
    ...
    <div style={{ minHeight: "100vh", backgroundColor: "#EDEFF2" }}>
      ...
      {showModalPagamento && (
        <ModalPagamento
          ...
          enderecoSelecionadoCliente={enderecoSelecionadoCliente}
        />
      )}
        ...
        <Row>
          ...
          <Col md={4}>
            ...
            <button
              ...
              style={{ backgroundColor: "#34495E" }}
              onClick={() => {
                if(enderecoSelecionadoCliente != undefined && enderecoSelecionadoCliente != null){
